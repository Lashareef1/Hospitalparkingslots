...
@RequestScoped
@FacesValidator("com.java.EmailVerify")
public class ExistingEmailverify implements Validator, Serializable {
	 private static final String EMAIL_REGEX =
		        "^[_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@[a-zA-Z]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$";
	...
	public void validate(FacesContext context, UIComponent comp, Object value) throws ValidatorException {
		...
		try {
			...
			if (count == 1) {
				FacesMessage msg = new FacesMessage("Already registered this Email",
						uniqueColumn);
	 ...
	 if (!email.matches(EMAIL_REGEX)) {
         FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Invalid email format",
